...
// [NEW] Helper mirroring runQuery() above, but for the tree round-trip verifier instead of a query it loads a file, builds the tree, and returns the VerificationResult so tests can assert on result.passed / result.issues directly, the same pattern runQuery() already uses for query results.
VerificationResult runVerification(const std::string& file) {
    parser p;
    if (!p.loadFile(file)) {
        ADD_FAILURE() << "Failed to load file: " << file;
    ...
    return verifyTree(p.getRoot(), p.getJsonData(), p.getTypeIndex());
}
...
/*Suite 6: tree round-trip verification (tree_verifier.h)
